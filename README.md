...
* [`src/cljs/pouchdb_fx_examples/`](src/cljs/pouchdb_fx_examples/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/pouchdb_fx_examples/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `pouchdb-fx-examples` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/pouchdb_fx_examples/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns pouchdb-fx-examples.example
  (:require [pouchdb-fx-examples.config :as config])
